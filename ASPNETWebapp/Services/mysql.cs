...
namespace MyProject.Services
{
    public static class DatabaseConnector
    {
        private const string ConnectionString =
            "Server=db;Database=web_app;User=app_user;Password=app_password;SslMode=None;";
        ...
        private static (int affectedRows, long insertId) Exec(
            string sql,
        ...
        public static object InsertUser(
            ...
            bool isAdmin = false)
        {
            string sql = "INSERT INTO users (username, firstname, lastname, email, password, avatar, is_admin) " +
                            "VALUES ('" + username + "', '" + firstname + "', '" + lastname + "', '" +
                            email + "', '" + password + "', '" + avatar + "', " + (isAdmin ? 1 : 0) + ")";
        ...
        {
            string sql = "UPDATE users SET is_admin = NOT is_admin WHERE id = " + id;
        ...
        {
            var (affected, _) = Exec("DELETE FROM users WHERE id = " + id);
        ...
        public static object CreateBlog(
            ...
            bool isPrivate)
        {
            string author = string.IsNullOrWhiteSpace(authorName)
                ? "anonimo"
                : authorName;

            string sql = "INSERT INTO blogs (title, content, author, url, is_private) VALUES ('" +
        ...
        {
            string sql = "INSERT INTO comments (blog_id, writer, comment) VALUES (" +
                            blogId + ", '" + writer + "', '" + comment + "')";
        ...
        {
            return Query("SELECT * FROM comments WHERE blog_id = " + blogId);
        }

        public static List<Dictionary<string, object>> ListFilesByCommentId(int commentId)
        {
            return Query("SELECT * FROM comment_files WHERE comment_id = " + commentId);
        }
