...
namespace MyProject.Services
{
    public static class DatabaseConnector
    {
        private const string ConnectionString =
            "Host=db;Port=5432;Database=web_app;Username=admin;Password=admin";
        ...
        {
            string sql =
                "INSERT INTO users (username, firstname, lastname, email, password, avatar, is_admin) VALUES ('" +
                username + "', '" +
                firstname + "', '" +
                lastname + "', '" +
                email + "', '" +
                password + "', '" +
                avatar + "', " +
                (isAdmin ? "TRUE" : "FALSE") + ") RETURNING id";
        ...
        {
            return Query("SELECT * FROM blogs WHERE is_private = FALSE");
        }
